...
function changePizzaSizes(size) {  //manually  setting pizza sizes
  switch (size) {
    case "1":
      newwidth = 25;
      break;
...
var randomPizzas = document.getElementsByClassName("randomPizzaContainer");
  ...
  for (var i = 0; i<randomPizzas.length; i++) {
    randomPizzas[i].style.width  = newwidth + "%";  //using % for setting width
  }
}
...
var pizzasDiv = document.getElementById("randomPizzas");// calculating pizzasDiv only once
for (var i = 2; i < 100; i++) {
...
var items = document.getElementsByClassName('mover'); //took this query selector outside function updatePositions()
function updatePositions() {
  frame++;
  window.performance.mark("mark_start_frame");

  var scrollPosition = document.body.scrollTop / 1250; //accessing this scrollPosition only once
  var phase;
...
document.addEventListener('DOMContentLoaded', function() {
  var cols = 8;
  var s = 256;
  var movingPizzas = document.getElementById('movingPizzas1');   //accessing this movingPizza only once
  for (var i = 0; i < 100; i++) {     //reduced number of pizzas
